...
class ThreadXHWDefIncludeNotFoundException(Exception):
    def __init__(self, hwdef, includer):
        self.hwdef = hwdef
        self.includer = includer


class ThreadXHWDef(hwdef.HWDef):
...
''')
    ...
    def write_mcu_config(self, f):
        ...
        f.write('#define THREADX_BOARD_NAME "%s"\n' % os.path.basename(os.path.dirname(self.hwdef[0])))
        f.write('// MCU type (threadX define)\n')
...
''')
        ...
        if self.have_type_prefix('CAN') and not using_threadx_can:
            self.enable_can(f)
    ...
    def write_UART_config(self, f):
        ...
        for idx, dev in enumerate(serial_list):
            if dev == 'EMPTY':
                ...
                f.write(
                    '#define HAL_SERIAL%s_DRIVER threadX::UARTDriver serial%sDriver(%u)\n'
                    % (idx, idx, sdev))
                sdev += 1
        ...
        if 'IOMCU_UART' in self.config:
            ...
            if self.config['IOMCU_UART'][0]:
                ...
                f.write(
                    '#define HAL_UART_IO_DRIVER constexpr threadX::UARTDriver &uart_io = serial%sDriver;\n' % (index)
                )
...
#                    ret += "# End included file (%s)" % include_filepath
    ...
    def process_line_undef(self, line, depth, a):
        ...
        super().process_line_undef(line, depth, a)
    ...
    def process_line_env(self, line, depth, a):
        ...
        super().process_line_env(line, depth, a)
...
''' % (description, content, description))
    ...
    def is_periph_fw_unprocessed_file(self, hwdef, includer=None):
        ...
        if not os.path.exists(hwdef):
            raise ThreadXHWDefIncludeNotFoundException(
    ...
    def run(self):
        ...
        self.env_vars['THREADX_BUILD_FLAGS'] = ' '.join(self.build_flags)
