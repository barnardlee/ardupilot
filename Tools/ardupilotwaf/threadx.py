...
def _load_dynamic_env_data(bld):
    bldnode = bld.bldnode.make_node('modules/threadX')
...
@feature('tx_ap_library', 'tx_ap_program')
@before_method('process_source')
def tx_dynamic_env(self):
...
@feature('tx_ap_program')
@after_method('process_source')
def threadx_firmware(self):
...
def load_env_vars(env):
    ...
    if env.DEBUG or env.DEBUG_SYMBOLS:
        env.THREADX_BUILD_FLAGS += ' ENABLE_DEBUG_SYMBOLS=yes'
    if env.ENABLE_ASSERTS:
        env.THREADX_BUILD_FLAGS += ' ENABLE_ASSERTS=yes'
    if env.ENABLE_MALLOC_GUARD:
        env.THREADX_BUILD_FLAGS += ' ENABLE_MALLOC_GUARD=yes'
    if env.ENABLE_STATS:
        env.THREADX_BUILD_FLAGS += ' ENABLE_STATS=yes'
    if env.ENABLE_DFU_BOOT and env.BOOTLOADER:
        env.THREADX_BUILD_FLAGS += ' USE_ASXOPT=-DCRT0_ENTRY_HOOK=TRUE'
    if env.AP_BOARD_START_TIME:
        env.THREADX_BUILD_FLAGS += ' AP_BOARD_START_TIME=0x%x' % env.AP_BOARD_START_TIME
...
def setup_optimization(env):
    ...
    env.THREADX_BUILD_FLAGS += ' USE_COPT=%s' % OPTIMIZE
...
def configure(cfg):
    ...
    env.AP_PROGRAM_FEATURES += ['tx_ap_program']

    kw = env.AP_LIBRARIES_OBJECTS_KW
    kw['features'] = Utils.to_list(kw.get('features', [])) + ['tx_ap_library']

    env.TX_ROOT = srcpath('modules/threadX')
    env.CC_ROOT = srcpath('modules/CrashDebug/CrashCatcher')
    env.AP_HAL_ROOT = srcpath('libraries/AP_HAL_threadX')
    env.BUILDDIR = bldpath('modules/threadX')
    env.BUILDROOT = bldpath('')
    env.SRCROOT = srcpath('')
    env.PT_DIR = srcpath('Tools/ardupilotwaf/threadx/image')
    env.MKFW_TOOLS = srcpath('Tools/ardupilotwaf')
    env.UPLOAD_TOOLS = srcpath('Tools/scripts')
    env.THREADX_SCRIPTS = srcpath('libraries/AP_HAL_threadX/hwdef/scripts')
    ...
    env.TX_ROOT_REL = os.path.relpath(env.TX_ROOT, env.BUILDROOT)
    ...
    mk_custom = srcpath('libraries/AP_HAL_threadX/hwdef/%s/threadx_board.mk' % env.BOARD)
    mk_common = srcpath('libraries/AP_HAL_threadX/hwdef/common/threadx_board.mk')
...
def generate_hwdef_h(env):
    '''run threadx_hwdef.py'''
    import subprocess
    if env.BOOTLOADER:
        if len(env.HWDEF) == 0:
            env.HWDEF = os.path.join(env.SRCROOT, 'libraries/AP_HAL_threadX/hwdef/%s/hwdef-bl.dat' % env.BOARD)
        else:
        ...
        if len(env.HWDEF) == 0:
            env.HWDEF = os.path.join(env.SRCROOT, 'libraries/AP_HAL_threadX/hwdef/%s/hwdef.dat' % env.BOARD)
    ...
    if env.AP_SIGNED_FIRMWARE:
        ...
        print(env.BOOTLOADER_OPTION)
    hwdef_script = os.path.join(env.SRCROOT, 'libraries/AP_HAL_threadX/hwdef/scripts/threadx_hwdef.py')
...
def build(bld):
    ...
    hwdef_rule="%s '%s/hwdef/scripts/threadx_hwdef.py' -D '%s' --params '%s' '%s'" % (
    ...
    bld(
        ...
        rule="touch Makefile && BUILDDIR=${BUILDDIR_REL} BUILDROOT=${BUILDROOT} CRASHCATCHER=${CC_ROOT_REL} THREADX=${TX_ROOT_REL} AP_HAL=${AP_HAL_REL} ${THREADX_BUILD_FLAGS} ${THREADX_BOARD_NAME} ${MAKE} pass -f '${BOARD_MK}'",
        group='dynamic_sources',
        target=bld.bldnode.find_or_declare('modules/threadX/include_dirs')
    )

    bld(
        # create the file modules/ChibiOS/include_dirs
        rule="echo // BUILD_FLAGS: ${BUILDDIR_REL} ${BUILDROOT} ${CC_ROOT_REL} ${TX_ROOT_REL} ${AP_HAL_REL} ${THREADX_BUILD_FLAGS} ${THREADX_BOARD_NAME} ${HAL_MAX_STACK_FRAME_SIZE} > threadx_flags.h",
        group='dynamic_sources',
        target=bld.bldnode.find_or_declare('threadx_flags.h')
    )
    ...
    common_src = [bld.bldnode.find_or_declare('hwdef.h'),
                  ...
                  bld.bldnode.find_or_declare('modules/threadX/include_dirs')]
    common_src += bld.path.ant_glob('libraries/AP_HAL_threadX/hwdef/common/*.[ch]')
    common_src += bld.path.ant_glob('libraries/AP_HAL_threadX/hwdef/common/*.mk')
    common_src += bld.path.ant_glob('modules/threadX/os/hal/**/*.[ch]')             #need to be checked!!!
    common_src += bld.path.ant_glob('modules/threadX/os/hal/**/*.mk')               #need to be checked!!!
    ...
    if bld.env.ENABLE_CRASHDUMP:
        tx_task = bld(
            ...
            rule="BUILDDIR='${BUILDDIR_REL}' BUILDROOT='${BUILDROOT}' CRASHCATCHER='${CC_ROOT_REL}' THREADX='${TX_ROOT_REL}' AP_HAL=${AP_HAL_REL} ${THREADX_BUILD_FLAGS} ${THREADX_BOARD_NAME} ${HAL_MAX_STACK_FRAME_SIZE} '${MAKE}' -j%u lib -f '${BOARD_MK}'" % bld.options.jobs,
            group='dynamic_sources',
            source=common_src,
            target=[bld.bldnode.find_or_declare('modules/threadX/libtx.a'), bld.bldnode.find_or_declare('modules/threadX/libcc.a')]
        )
    else:
        tx_task = bld(
            # build libch.a from ChibiOS sources and hwdef.h
            rule="BUILDDIR='${BUILDDIR_REL}' BUILDROOT='${BUILDROOT}' THREADX='${TX_ROOT_REL}' AP_HAL=${AP_HAL_REL} ${THREADX_BUILD_FLAGS} ${THREADX_BOARD_NAME} ${HAL_MAX_STACK_FRAME_SIZE} '${MAKE}' -j%u lib -f '${BOARD_MK}'" % bld.options.jobs,
            group='dynamic_sources',
            source=common_src,
            target=bld.bldnode.find_or_declare('modules/threadX/libtx.a')
        )
    tx_task.name = "threadX_lib"
    DSP_LIBS = {
    ...
    if bld.env.CORTEX in DSP_LIBS:
        ...
        shutil.copyfile(os.path.join(bld.env.SRCROOT,'libraries/AP_GyroFFT/CMSIS_5/lib',libname),
                        os.path.join(bld.env.BUILDROOT,'modules/threadX/libDSP.a'))
        bld.env.LIB += ['DSP']
    bld.env.LIB += ['tx']
    bld.env.LIBPATH += ['modules/threadX/']
    if bld.env.ENABLE_CRASHDUMP:
        bld.env.LINKFLAGS += ['-Wl,-whole-archive', 'modules/threadX/libcc.a', '-Wl,-no-whole-archive']
